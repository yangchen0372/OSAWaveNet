...
if __name__ == '__main__':
    ...
    LOG_DIR = os.path.join(PROJECT_HOME, 'TrainLog', 'OSAWaveNet_LEVIR256', PROJECT_RUN_TIME) # 日志保存路径
    ...
    TRAIN_DATASET = LEVIR256(flag='train')
        ...
        if current_epoch % 1 == 0 or current_epoch == train_epoch:
            ...
            with torch.no_grad():
                for iter, batch in enumerate(TEST_DATALOADER,start=1):
                    ...
                    print('\r[{}] Epoch-Test {}\{} best_iou:{}'.format(current_epoch, iter, len(TEST_DATALOADER),best_iou), end='')
                epoch_metrics = bce_metrics.cm2score()
                if epoch_metrics['IoU'] > best_iou:
                    best_iou = epoch_metrics['IoU']
                    logger.save_checkpoint(model, 'EXPMODEL_LEVIR256_best.pt')
                logger.write(
                    log_str='test metrics: ' + str(bce_metrics.cm2score())
                )
                logger.save_checkpoint(model, 'EXPMODEL_LEVIR256_{}.pt'.format(current_epoch))
