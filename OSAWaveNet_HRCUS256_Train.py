...
if __name__ == '__main__':
    ...
    LOG_DIR = os.path.join(PROJECT_HOME, 'TrainLog', 'OSAWaveNet_HRCUS256', PROJECT_RUN_TIME) # 日志保存路径
    ...
    TRAIN_DATASET = HRCUS256(flag='train')
        ...
        if current_epoch % 1 == 0 or current_epoch == train_epoch:
            ...
            with torch.no_grad():
                ...
                if epoch_metrics['IoU'] > best_iou:
                    ...
                    logger.save_checkpoint(model, 'EXPMODEL_HRCUS256_best.pt')
                logger.write(
                    log_str='test metrics: ' + str(bce_metrics.cm2score())
                )
                logger.save_checkpoint(model, 'EXPMODEL_HRCUS256_{}.pt'.format(current_epoch))
