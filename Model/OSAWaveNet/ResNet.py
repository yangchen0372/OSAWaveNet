...
def _resnet(block, layers, pretrained_path=None):
    model = ResNet(block, layers)
    if pretrained_path is not None:
        state_dict = torch.load(pretrained_path,weights_only=False)
        print("ResNet->loading imagenet pretrained")
        model.load_state_dict(state_dict, strict=False)
        print("ResNet->loaded imagenet pretrained")
    return model

def resnet18(pretrained_path=None):
    return _resnet(BasicBlock, [2, 2, 2, 2], pretrained_path)

def resnet34(pretrained_path=None):
    return _resnet(BasicBlock, [3, 4, 6, 3],pretrained_path)

def resnet50(pretrained_path=None):
    return _resnet(Bottleneck, [3, 4, 6, 3], pretrained_path)

def resnet101(pretrained_path=None):
    return _resnet(Bottleneck, [3, 4, 23, 3], pretrained_path)

def resnet152(pretrained_path=None):
    return _resnet(Bottleneck, [3, 8, 36, 3], pretrained_path)
